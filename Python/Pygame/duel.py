...
class Ship:
  ...
  """
  def __init__(self, x, y, velocity_x, velocity_y, health):
    """
...
class Scout(Ship):
  """
  Klasse som representerer et scout-skip og arver fra klassen Ship
  ...
  ...
  scout_bullet_width : int
    bredden på kulene til scout-skip
  scout_bullet_height : int
    høyden på kulene til scout-skip

  Metoder
  -------
  draw()
    Tegner scout-skipet på skjermen ved gitte koordinater og bilde
  """
  def __init__(self, x, y, velocity_x, velocity_y, health, scout_bullet_width, scout_bullet_height):
    """
    Konstruerer alle de nødvendige attributtene for et scout-objekt
    ...
    super().__init__(x, y, velocity_x, velocity_y, health)
    self.scout_bullet_width = scout_bullet_width
    self.scout_bullet_height = scout_bullet_height
  ...
  def draw(self):
        ...
        None     
        """
        #Tegner et scout skip på skjermen ved gitte koordinater
        WINDOW.blit(SCOUT_SHIP, (self.x, self.y))

class Heavy(Ship):
  """
  Klasse som representerer et heavy-skip og arver fra klassen Ship
  ...
  ...
  health : int
    heltall som representerer hvor mange liv man har
  heavy_bullet_width : int
    bredden på kulene til heavy-skip
  heavy_bullet_height : int
    høyden på kulene til heavy-skip
  ...
  def __init__(self, x, y, velocity_x, velocity_y, health, heavy_bullet_width, heavy_bullet_height):
    """
    Konstruerer alle de nødvendige attributtene for et heavy-objekt
    ...
    #Arver fra klassen Ship
    super().__init__(x, y, velocity_x, velocity_y, health)
    self.heavy_bullet_width = heavy_bullet_width
    self.heavy_bullet_height = heavy_bullet_height


  def draw(self):
        """
        Tegner heavy-skipet på skjermen ved gitte koordinater og bilde   
        ...
        None     
        """
        #Tegner et heavy ship ved gitte koordinater
        WINDOW.blit(HEAVY_SHIP, (self.x, self.y))

class Bot(Scout):
  """
  Klasse som representerer et bot-skipet og arver fra klassen Scout
  ...
  ...
  def __init__(self, x, y, velocity_x, velocity_y, health, scout_bullet_width, scout_bullet_height):
    ...
    scout_bullet_height : int
        høyden på kulene til scout-skip  
    """
    #Arver fra klassen Scout
    super().__init__(x, y, velocity_x, velocity_y, health, scout_bullet_width, scout_bullet_height)
  ...
  def draw(self):   
    ...
    None     
    """
    #Tegner et bot ship ved gitte koordinater
    WINDOW.blit(BOT_SHIP, (self.x, self.y))   
...
def main():
  ...
  #Du har valgt scout
  if ship_type == "scout": 
     #Lager et objekt av klassen Scout
     player = Scout(100,300,10,10, 3, 10, 4)

  #Du har valgt heavy
  else:
     #Lager et objekt av klassen Heavy
     player = Heavy(100,300,4,4, 4, 18,8)
     
  #Lager et objekt av klassen Bot
  bot = Bot(700,300,5,5, 3, 10, 4)
...
def bullets(player, bot, player_rect, bot_rect, ship_type):
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN and len(stats.bullets_list_player) < stats.max_bullets:
            ...
            if event.key == pygame.K_LCTRL:
                #Lager en scout bullet
                if(ship_type=="scout"):
                    bullet = pygame.Rect(player_rect.x + SPACESHIP_WIDTH/2, player_rect.centery - player.scout_bullet_height/2, player.scout_bullet_width, player.scout_bullet_height)
                #Lager en heavy bullet
                else:
                    bullet = pygame.Rect(player_rect.x + SPACESHIP_WIDTH/2, player_rect.centery - player.heavy_bullet_height/2, player.heavy_bullet_width, player.heavy_bullet_height)
    ...
    if(stats.framecounter%60==0):
        ...
        bullet = pygame.Rect(bot_rect.x + SPACESHIP_WIDTH/2, bot_rect.centery - bot.scout_bullet_height/2, bot.scout_bullet_width, bot.scout_bullet_height)
