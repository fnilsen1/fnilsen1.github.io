...
class Scout(Ship):
  ...
  scout_bullet_width = 10
  scout_bullet_height = 4
  def __init__(self, x, y, velocity_x, velocity_y, health):
    """
...
class Heavy(Ship):
  ...
  draw()
    Tegner scout-skipet på skjermen ved gitte koordinater og bilde
  """  
  heavy_bullet_width = 18
  heavy_bullet_height= 8
...
def main():
  ...
  if ship_type == "scout": 
     ...
     player = Scout(100,300,10,10, 3)

  #Du har valgt heavy
  else:
     #Lager et objekt av klassen Heavy
     player = Heavy(100,300,4,4, 4)
     
  #Lager et objekt av klassen Bot
  bot = Bot(700,300,5,5, 3)
...
def bullets(player, bot, player_rect, bot_rect, ship_type):
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN and len(stats.bullets_list_player) < stats.max_bullets:
            ...
            if event.key == pygame.K_LCTRL:
                ...
                if(ship_type=="scout"):
                    bullet = pygame.Rect(player_rect.x + SPACESHIP_WIDTH/2, player_rect.centery - Scout.scout_bullet_height/2, Scout.scout_bullet_width, Scout.scout_bullet_height)
                #Lager en heavy bullet
                else:
                    bullet = pygame.Rect(player_rect.x + SPACESHIP_WIDTH/2, player_rect.centery - Heavy.heavy_bullet_height/2, Heavy.heavy_bullet_width, Heavy.heavy_bullet_height)
    ...
    if(stats.framecounter%60==0):
        ...
        bullet = pygame.Rect(bot_rect.x + SPACESHIP_WIDTH/2, bot_rect.centery - Bot.scout_bullet_width/2, Bot.scout_bullet_width, Bot.scout_bullet_height)
